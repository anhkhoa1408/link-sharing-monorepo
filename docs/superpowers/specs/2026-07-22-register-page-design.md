...
- Keep the user on `/register` after success and instruct them to check their email for account confirmation.
...
- Pending, success, and API-error state.
...
5. A successful response clears stale API errors and displays: “Check your email to confirm your account.”
6. The user remains on `/register`; no session is persisted and no automatic login occurs.
7. The “Login” link navigates to `/login`.
